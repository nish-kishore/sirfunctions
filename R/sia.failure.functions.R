...
#' @import dplyr ggplot2 scales ggpubr
...
surv_plot_func <- function(region,
  ...
  a <- dplyr::filter(tmp, round.num.sia == 1) |>
    ggplot2::ggplot(aes(x = timetofirstcase, y = surv.01, group = vaccine.type, color = vaccine.type)) +
    #geom_point(alpha = 0.5, fill = NA) +
    ggplot2::geom_point() +
    ggplot2::geom_line() +
    ggplot2::facet_wrap(~ctry, drop = F, ncol = 3) +
    ggplot2::theme_bw() +
    ggplot2::scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
    ggplot2::scale_color_manual(values = color.surv.plots) +
    ggplot2::labs(x = "Days to first case", y = "Percent of campaigns that succeeded",
                  shape = paste0("Failed before ",breakthrough_middle_date," days"),
                  color = "Vaccine\nType", title = "Round 1") +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1)) +
    ggplot2::coord_cartesian(ylim = c(0.8,1))

  b <- dplyr::filter(tmp, round.num.sia == 2) |>
    ggplot2::ggplot(aes(x = timetofirstcase, y = surv.01, group = vaccine.type, color = vaccine.type)) +
    #geom_point(alpha = 0.5, fill = NA) +
    ggplot2::geom_point() +
    ggplot2::geom_line() +
  ...
  figure <- ggpubr::ggarrange(a, b, common.legend = T,
                              #labels = NULL, ncol = 2,
                              legend = "bottom", align = "hv",
                              font.label = list(size = 10, color = "black", face = "bold", family = NULL, position = "top"))

  p <- ggpubr::annotate_figure(figure, top = ggpubr::text_grob(region, color = "black", face = "bold", size = 14) ,
...
create_emergence_plots_v2 <- function(case.sia.fig.01,
  ...
  case.sia.fig.01.sias <- case.sia.fig.01 |>
    dplyr::filter(typeb == "SIA") |>
    dplyr::filter(place.admin.0 %in% ctry)


  case.sia.fig.01.detections <- case.sia.fig.01 |>
    dplyr::filter(typeb == "Detection") |>
    dplyr::mutate(virus.emergence = ifelse(measurement %in% c("cVDPV 1", "cVDPV 2", "cVDPV 3"), paste0(measurement, ": ", emergencegroup), measurement)) |>
    dplyr::filter(place.admin.0 %in% ctry)
  ...
  case.sia.fig.01.detections.01 <- case.sia.fig.01.detections |>
    dplyr::mutate(mon.yr = lubridate::floor_date(date, unit = "month")) |>
    dplyr::group_by(mon.yr, place.admin.1, measurement, emergencegroup, typea) |>
    dplyr::mutate(num_detect = n()) |>
    dplyr::ungroup() |>
    dplyr::select(place.admin.1, measurement, emergencegroup, mon.yr, typea, virus.emergence, num_detect) |>
    dplyr::distinct()
  ...
  if(nrow(case.sia.fig.01.detections) >= 500){
    ...
    if(nrow(detection.types) > 1){
      ...
      plot.1 <- ggplot2::ggplot() +
        ggplot2::geom_tile(data=case.sia.fig.01.sias,
                           aes(x = date, y = place.admin.1, fill = typea, width = 14)) +
        ggplot2::scale_x_date(breaks = "6 month", date_labels = "%m-%Y",
                              limits = c(as_date(min_date), as_date(end.date))) +
        ggplot2::ylab("Provinces") +
        ggplot2::xlab(NULL) +
        ggplot2::labs(title = ctry) +
        ggplot2::scale_fill_manual(name = "SIAs", values = color.sia.plots.2, limits = force) +
        ggplot2::geom_point(data = case.sia.fig.01.detections.01 |> dplyr::filter(measurement == "cVDPV 1"),
                            aes(x = mon.yr, y = place.admin.1, color = typea,
                                shape = virus.emergence, size = num_detect)) +
        ggplot2::scale_size_binned_area(
          limits = c(0, 50),
          breaks = c(0, 15, 30, 50)) +
        ggplot2::scale_shape_manual(values = 1:length(unique(case.sia.fig.01.detections$virus.emergence))) +
      ...
      plot.2 <- ggplot2::ggplot() +
        ...
        ggplot2::labs(title = ctry) +
        ggplot2::scale_fill_manual(name = "SIAs", values = color.sia.plots.2, limits = force) +
        ggplot2::geom_point(data = case.sia.fig.01.detections.01 |> dplyr::filter(measurement == "cVDPV 2"),
      ...
      plot <- ggplot2::ggplot() +
        ...
        ggplot2::labs(title = ctry) +
        ggplot2::scale_fill_manual(name = "SIAs", values = color.sia.plots.2, limits = force) +
        ggplot2::geom_point(data = case.sia.fig.01.detections.01,
                            aes(x = mon.yr, y = place.admin.1, color = typea,
                                shape = virus.emergence, size = num_detect)) +
        ggplot2::scale_size_binned_area(
          limits = c(0, 50),
...
#' @param breakthrough_middle_date int number of days to set cutoff between early and late breakthrough
#' @param breakthrough_max_date int maximum number of days a case could be considered breakthrough
create_dist_failures <- function(case.sia.02,
                                 method,
                                 ctry.region.2 = NULL,
                                 breakthrough_min_date=load_parameters()$breakthrough_min_date,
                                 breakthrough_middle_date = load_parameters()$breakthrough_middle_date,
...
#' @description
#' a function to identify SIAs with failures
#' @import dplyr
#' @param case.sia.02 tibble df produced from create_case_sia_02
#' @param method str "yr" or "ctry_yr", determines whether to assess SIA failure by year or by country and year
#' @param breakthrough_min_date int minimum days after SIA to be considered breakthrough
#' @param breakthrough_middle_date int number of days to set cutoff between early and late breakthrough
#' @param breakthrough_max_date int maximum number of days a case could be considered breakthrough
create_sia_failure <- function(case.sia.02,
                               method,
                               ctry.region.2 = NULL,
                               breakthrough_min_date = load_parameters()$breakthrough_min_date,
                               breakthrough_middle_date = load_parameters()$breakthrough_middle_date,
                               breakthrough_max_date = load_parameters()$breakthrough_max_date){
  sia.failure <- create_case_sia_03(case.sia.02) %>%
    ...
    select(sia.sub.activity.code, yr.sia, place.admin.0, vaccine.type,
           round.num.sia, num.breakthrough.01, num.breakthrough.02) %>%
    distinct() %>%
    mutate(breakthrough.01 = ifelse(num.breakthrough.01>0, 1, 0),
           breakthrough.02 = ifelse(num.breakthrough.02>0, 1, 0))

  if(method == "yr"){
    #Any failure across the SIA geography by year
    sia.failure.yr <- sia.failure %>%
      group_by(yr.sia, vaccine.type, round.num.sia) %>%
      mutate(num.sia = n(),
             num.breakthrough.01 = sum(breakthrough.01),
             num.breakthrough.02 = sum(breakthrough.02)) %>%
      select(yr.sia, vaccine.type, round.num.sia, num.sia,
             num.breakthrough.01, num.breakthrough.02)%>%
      distinct() %>%
      mutate(per.break01 = round(num.breakthrough.01/num.sia*100),
             per.break02 = round(num.breakthrough.02/num.sia*100),
             num.break01.v1 = paste(num.breakthrough.01, num.sia, sep="/"),
             num.break02.v2 = paste(num.breakthrough.02, num.sia, sep="/"),
             braket = "(",
             per = "%)",
             per.break01.v2 = paste(braket, per.break01, per, sep=""),
             per.break02.v2 = paste(braket, per.break02, per, sep=""),
             per.break01.v3 = paste(num.break01.v1, per.break01.v2, sep=" "),
             per.break02.v3 = paste(num.break02.v2, per.break02.v2, sep=" ")) %>%
      select(yr.sia, round.num.sia, vaccine.type, per.break01.v3, per.break02.v3) %>%
      pivot_wider(names_from = vaccine.type, values_from=per.break01.v3:per.break02.v3) %>%
      arrange(yr.sia, round.num.sia) %>%
      rename(Year = yr.sia, "SIA Round" = round.num.sia,
             !!(paste0("mOPV2 breakthrough ",breakthrough_min_date,"-",breakthrough_middle_date," days")) := per.break01.v3_mOPV2,
             !!(paste0("nOPV2 breakthrough ",breakthrough_min_date,"-",breakthrough_middle_date," days")) := per.break01.v3_nOPV2,
             !!(paste0("tOPV breakthrough ",breakthrough_min_date,"-",breakthrough_middle_date," days")) := per.break01.v3_tOPV,
             !!(paste0("bOPV breakthrough ",breakthrough_min_date,"-",breakthrough_middle_date," days")) := per.break01.v3_bOPV,
    ...
    return(sia.failure.yr)

  }

  if(method == "ctry_yr"){
    #SIA failure by country and year

    #Any failure across the SIA geography by year
    sia.failure.ctry.yr <- sia.failure %>%
      group_by(place.admin.0, yr.sia, vaccine.type, round.num.sia) %>%
      mutate(num.sia = n(),
             num.breakthrough.01 = sum(breakthrough.01),
             num.breakthrough.02 = sum(breakthrough.02)) %>%
      select(place.admin.0, yr.sia, vaccine.type, round.num.sia, num.sia, num.breakthrough.01, num.breakthrough.02)%>%
      distinct() %>%
      mutate(per.break01 = round(num.breakthrough.01/num.sia*100),
             per.break02 = round(num.breakthrough.02/num.sia*100),
             num.break01.v1 = paste(num.breakthrough.01, num.sia, sep="/"),
             ...
             per.break01.v3 = paste(num.break01.v1, per.break01.v2, sep=" "),
             per.break02.v3 = paste(num.break02.v2, per.break02.v2, sep=" ")) %>%
      select(place.admin.0, yr.sia, round.num.sia, vaccine.type, per.break01.v3, per.break02.v3) %>%
      pivot_wider(names_from = vaccine.type, values_from=per.break01.v3:per.break02.v3) %>%
      arrange(round.num.sia, yr.sia) %>%
      rename(Country = place.admin.0, Year = yr.sia, "SIA Round" = round.num.sia,
             !!(paste0("mOPV2 breakthrough ",breakthrough_min_date,"-",breakthrough_middle_date," days")) := per.break01.v3_mOPV2,
             !!(paste0("nOPV2 breakthrough ",breakthrough_min_date,"-",breakthrough_middle_date," days")) := per.break01.v3_nOPV2,
             !!(paste0("tOPV breakthrough ",breakthrough_min_date,"-",breakthrough_middle_date," days")) := per.break01.v3_tOPV,
             !!(paste0("bOPV breakthrough ",breakthrough_min_date,"-",breakthrough_middle_date," days")) := per.break01.v3_bOPV,
...
#' @param start.date date starting date of analysis
#' @param end.date date end date of analysis
#' @param start_year int starting year of SIAs, used to restrict to recent SIAs
f.geompoint.case <- function(.x,
                             case.sia.fig.01.sias,
                             case.sia.fig.01.detections,
                             start.date,
                             end.date,
...
#' @description
#' a function to build a flextable summarizing SIA failure at the district level
#' @import flextable readr dplyr
#' @param k str WHO region name: AFRO, EMRO, etc.
#' @param dist.ctry.failure.tab.2 tibble df detailing SIA rounds with breakthrough by district, summarized at country level
#' @param breakthrough_min_date int minimum days after SIA to be considered breakthrough
#' @param breakthrough_middle_date int number of days to set cutoff between early and late breakthrough
#' @param breakthrough_max_date int maximum number of days a case could be considered breakthrough
f.dist.ctry.failure.tab <- function(k,
                                    dist.ctry.failure.tab.2,
                                    breakthrough_min_date = load_parameters()$breakthrough_min_date,
                                    breakthrough_middle_date = load_parameters()$breakthrough_middle_date,
                                    breakthrough_max_date = load_parameters()$breakthrough_max_date) {
  ...
  flextable::set_header_df(x = flextable::flextable(dist.ctry.failure.tab.2 |>
                                                      dplyr::filter(WHO_REGION %in% k), col_keys = col_keys_vec),
                           mapping = data.frame(key = names1, values = names2, stringsAsFactors = FALSE), key = "key") |>

    bg(j = 4:11,
       bg = function(x){
         out <- rep("transparent", length(x))
         out[readr::parse_number(gsub("(?<=\\()[^()]*(?=\\))(*SKIP)(*F)|.", "", x, perl=T)) < 5 &
               readr::parse_number(gsub("(?<=\\()[^()]*(?=\\))(*SKIP)(*F)|.", "", x, perl=T)) > 0 ] <- "#fcbba1"
         out[readr::parse_number(gsub("(?<=\\()[^()]*(?=\\))(*SKIP)(*F)|.", "", x, perl=T)) >= 5 &
               readr::parse_number(gsub("(?<=\\()[^()]*(?=\\))(*SKIP)(*F)|.", "", x, perl=T)) <= 10 ] <- "#fb6a4a"
         out[readr::parse_number(gsub("(?<=\\()[^()]*(?=\\))(*SKIP)(*F)|.", "", x, perl=T)) >10 ] <- "#ef3b2c"

         out}) |>
    flextable::set_header_labels("Detections 21-180 days" = 4:7, "Detections 181-365 days"= 8:11) |>
    ...
    flextable::width(j = 4:11, width = 1.2) |>
    ...
    flextable::add_footer(Year = paste0("Results are restricted to SIAs with >=",breakthrough_middle_date," days of observation time")) |>
    flextable::merge_at(j = 1:11, part = "footer") |>
...
f.sia.failure.ctry.yr <- function(k,
  ...
  flextable::set_header_df(x = flextable::flextable(sia.failure.ctry.yr.2 |>
                                                      dplyr::filter(WHO_REGION %in% k), col_keys = col_keys_vec),
                           mapping = data.frame(key = names1, values = names2, stringsAsFactors = FALSE), key = "key") |>
    bg(j = 4:11,
       bg = function(x){
    ...
    flextable::theme_vanilla() |>
    flextable::add_footer(Year = paste0("Results are restricted to SIAs with >=",breakthrough_middle_date," days of observation time")) |>
    flextable::merge_at(j = 1:7, part = "footer") |>
    flextable::fontsize(size = 10, part = "all") |>
    flextable::align(align = "center", part = "all") |>
    flextable::align(j = 1:2, align = "left", part = "all") |>
    flextable::valign(j = 1, valign = "middle", part = "body") %>%
