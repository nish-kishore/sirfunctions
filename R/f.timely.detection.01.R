...
f.timely.detection.01 <- function(
  ...
  afp.pos.detect.01 <- afp.data %>%
    ungroup() %>%
    filter(!cdc.classification.all.2 %in% c("COMPATIBLE" ,"NPAFP", "PENDING", "UNKNOWN", "VAPP", "LAB PENDING")) %>%
    select(epid, ctry, year, cdc.classification.all.2, date, datenotificationtohq) %>%
    mutate(
