test_that("testing f.npafp.rate.01()", {
  ...
  country_data <- sirfunctions::extract_country_data(.country = country_name,
                                                     .raw.data = raw_data)
  afp_data <- country_data$afp.all.2
  c_afp_data <- afp_data |>
    select(adm0guid, adm1guid, adm2guid, date, cdc.classification.all2, epid,
           ctry, prov, dist)
  cpop_data <- raw_data$ctry.pop |>
    select(ADM0_NAME, adm0guid, u15pop, year, datasource) |>
    rename("ctry" = "ADM0_NAME") |>
    filter(ctry == str_trim(str_to_upper(country_name)),
  ...
  expect_no_error(f.npafp.rate.01(afp.data = afp_data,
                                  pop.data = cpop_data,
  ...
  end_date <- "2022-12-31"
  expect_error(f.npafp.rate.01(afp.data = afp_data,
                               pop.data = cpop_data,
                               start.date = start_date,
                               end.date = end_date,
                              spatial.scale = "ctry"),
  ...
  # Check if the appropriate columns are found
